...
int estim2ColonneOuverte(  struct config *conf ) // fonction pareille a estim2 en prenant en considération les colonnes ouvertes
{
...
// estimation basée sur le nb de pieces et l'occupation en augmentant le coefficient de l'occupation du centre
int estim55(  struct config *conf )
{
	...
	ScrDisp = occCentreB - occCentreN;
  // nous avons multiplié ScrDisp par 2 pour lui donner un poid parraport à estim5
...
int estim7( struct config *conf )
{
	...
	if ( num_coup < 15 )			// 1ere phase (début)
	   return estim55(conf);
	if ( num_coup >= 15 && num_coup < 25 )	// fin de première phase
	   return estim2(conf);
...
//********************************************
// Vérifier les colonnes ouvertes et semi Ouvertes
//************************************************
